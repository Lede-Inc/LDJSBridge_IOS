...
- (void)isAppInstalled:(LDJSInvokedUrlCommand *)command
{
    ...
    LDJSPluginResult *pluginResult =
        [LDJSPluginResult resultWithStatus:LDJSCommandStatus_OK messageAsBool:isInstalled];
...
- (void)isAppInstalledBatch:(LDJSInvokedUrlCommand *)command
{
    ...
    LDJSPluginResult *pluginResult =
        [LDJSPluginResult resultWithStatus:LDJSCommandStatus_OK messageAsArray:arr_result];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}
...
{
    NSString *str_jumpurl =
        [NSString stringWithFormat:@"%@://%@", [command jsonParamForkey:@"appID"],
                                   [command jsonParamForkey:@"paramsStr"]];
