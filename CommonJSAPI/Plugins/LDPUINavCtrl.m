...
#define ENABLE_VIEWS ([NSDictionary dictionaryWithObjects:@[@"JSAboutCtrl"] forKeys:@[@"about"]])
 ...
 *@param style  Webview的样式(只对target＝1有效) 0:顶部标题栏（默认）1:顶部标题栏无分享 2:加底部工具栏 3:加底部工具栏顶部无分享；
 */
-(void)openLinkInNewWebView:(LDJSInvokedUrlCommand *)command{
    ...
    switch (target) {
        ...
        case 1:
        {
            ...
            [self.viewController.navigationController pushViewController:newWebViewCtrl animated:YES];
...
-(void)showDialog:(LDJSInvokedUrlCommand *)command{
    ...
    UIAlertView *confirmAlert = [[UIAlertView alloc] initWithTitle:title message:text
                                                 delegate:self
                                                 cancelButtonTitle:(needCancelBtn?(cancelBtnText&&cancelBtnText&&![cancelBtnText isEqualToString:@""]?cancelBtnText:@"取消"):(nil))
                                                 otherButtonTitles:(needOkBtn?(okBtnText&&![okBtnText isEqualToString:@""]?okBtnText:@"确定"):(nil)), nil];
