...
- (void)showActionSheet:(LDJSInvokedUrlCommand *)command
{
    ...
    UIActionSheet *actionSheet = [[UIActionSheet alloc]
                 initWithTitle:title
                      delegate:self
             cancelButtonTitle:(cancel && ![cancel isEqualToString:@""] ? cancel : @"取消")
        destructiveButtonTitle:(close && ![close isEqualToString:@""] ? close : @"确定")
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (actionSheet.tag == TAG_ACTIONSHEET) {
        ...
        NSString *callBackId =
            [[NSUserDefaults standardUserDefaults] objectForKey:NOTI_ACTIONSHEET];
        ...
        LDJSPluginResult *result = [LDJSPluginResult
            ...
            messageAsDictionary:[NSDictionary dictionaryWithObjectsAndKeys:
                                                  ...
                                                  [NSNumber numberWithLong:index], @"index", nil]];
...
- (void)pageVisibility:(LDJSInvokedUrlCommand *)command
{
    ...
    LDJSPluginResult *result =
        [LDJSPluginResult resultWithStatus:LDJSCommandStatus_OK messageAsBool:hidden ? NO : YES];
